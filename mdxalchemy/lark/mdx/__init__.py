...
from .analyzer import (
    retrieve_on_row_axis_statement,
    retrieve_on_column_axis_statement,
    retrieve_where_statement,
    retrieve_cube_source,
)
...
def analyze(mdx: str) -> QueryStatement:
    ...
    assert isinstance(tree,
                      lark.Tree), "Parsed tree should be of type lark.Tree"
    assert len(tree.children) == 1, "Parsed tree should have one child"
    root_tree: lark.Tree = tree.children[0]  # type: ignore
    query_statement = QueryStatement()
    assert root_tree.data == lark.Token(
        "RULE", QUERY_STATEMENT), "Root tree should be of type query_statement"
